...
int gettingUserNameforministatement(int clientFD, char* buff) 
{
 int n;
 int toRet = -1;
...
void processPoliceRequests(int clientFD, struct userInfo userId) 
{
 int n;
 char *buff = NULL;
 sendMsgToClient(clientFD, "Enter Customer username for Mini-Statement, B for Available Balance of all users and exit to terminate");
 ...
 while(1) 
	  ...
	  break;
         ...
         { 
          int ret = gettingUserNameforministatement(clientFD, buff);

          if(ret == 1 )
            {
             printMiniStatement(clientFD,buff);
            }
...
int main(int argc, char **argv) 
{
 ...
 //binding of the socket with the server logistics which are in sockaddr_in serv_addr
